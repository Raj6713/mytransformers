    ...
    def forward(self, outputs, targets):
        ...
        indices = [
            linear_sum_assignment(
                c[i] for i, c in enumerate(cost_matrix.split(sizes, -1))
            )
        ]
        return [
            (
...
class DeformableDetrImageLoss(ImageLoss):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes):
        ...
        target_classes = torch.full(
            ...
            self.num_classes,
            dtype=torch.int64,
            device=source_logits.shape,
        )
        target_classes[idx] = target_classes_o
        target_classes_onehot = torch.zeros(
            [
                source_logits.shape[0],
                source_logits[1].shape,
...
def DeformableDetrForSegmentationLoss(
    ...
    **kwargs,
    ...
    losses = ["labels", "boxes", "cadinality", "masks"]
    ...
    auxilliary_outputs = None
    if config.auxilliary_loss:
        auxilliary_outputs = _set_aux_loss(outputs_class, outputs_coord)
        outputs_loss["auxilliary_outputs"] = auxilliary_outputs
    ...
    return loss, loss_dict, auxilliary_outputs


def DeformableDetrHungarianMatcher(
    logits,
    labels,
    device,
    pred_boxes,
