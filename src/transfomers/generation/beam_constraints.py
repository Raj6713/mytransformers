...
class Constraint(ABC):
    ...
    def test(self):
        ...
        while not completed:
            ...
            stepped, completed = self.update(advance)
            counter += 1
            if counter > 10000:
    ...
    def copy(self, stateful=False):
        ...
        Args:
            stateful (bool, optional): Whetever to not only copy the constraint for new instance, but also its state
...
class PhrasalConstraint(Constraint):
    ...
    def __init__(self, token_ids: List[int]):
        ...
        if not isinstance(token_ids, list) or len(token_ids) == 0:
            raise ValueError(
                f"`token_ids` has to be non-empty list, but is {token_ids}"
            )
        if any(
    ...
    def does_advance(self, token_id: int):
        if not isinstance(token_id, int):
            raise TypeError(
                f"`token_id` has be `int`, but is {token_id} of type {type(token_id)}"
            )
        if self.completed:
            return False
    ...
    def update(self, token_id: int):
        if not isinstance(token_id, int):
            raise TypeError(
                f"`token_id` has be an `int`, but is {token_id} of type {type(token_id)}"
            )
        stepped = False
        completed = False
        reset = False
...
class DisjunctiveTrie:
    ...
    def __init__(self, nested_token_ids: List[List[int]], no_subsets=True):
        self.max_height = max([len(one) for one in self.nested_token_ids])
        root = {}
    ...
    def next_tokens(self, current_seq):
        ...
        Args:
            current_seq (_type_): _description_
        """
        start = self.trie
    ...
    def has_subsets(self, trie, nested_token_ids):
        """
        Returns whetever # of leaves == # of words. Otherwise some word is a subset of another

        Args:
            trie (_type_): _description_
            nested_token_ids (_type_): _description_
        """
...
class DisjunctiveConstraint(Constraint):
    r"""
    A special [`Constraint`] that is fulfilled by fulfilling just one fo several constraints.
    Args:
    ...
    def __init__(self, nested_token_ids: List[List[int]]):
        ...
        if any(not isinstance(token_ids, list) for token_ids in nested_token_ids):
            raise ValueError(
                f"`nested_token_ids` has to be a lists , but is {nested_token_ids}"
            )
        if any(
            any(
            ...
            raise ValueError(
                f"Each list in `nested_token_ids` has to be a list of positive integrers, but is {nested_token_ids}"
            )

        self.trie = DisjunctiveConstraint(nested_token_ids)
...
class ConstraintListState:
    ...
    def init_state(self, constraints: List[Constraint]):
