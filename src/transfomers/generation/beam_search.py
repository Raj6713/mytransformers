...
class BeamScorer(ABC):
    """
    Abstract base class for all beam scorers that are used fr [`~PretrainedModel.beam_search`] and
...
class BeamSearchScorer(BeamScore):
    r"""
    ...
    def __init__(
        ...
        if not isinstance(num_beams, int) or num_beams <= 1:
            raise ValueError(
                f"`num_beams` has to be an integer stricly greater than 1, but is {num_beams}. For `num_beams`==1"
        ...
        if (
            ...
            raise ValueError(
                "`num_beam_groups` has to be an integed smaller or equal than `num_beams` and `num_beams` has to be"
    ...
    def process(
        ...
        for batch_idx in range(batch_size):
            ...
            if self._done[batch_group_idx]:
                if self.num_beams < len(self._bram_hype[batch_group_idx]):
                    raise ValueError(
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(
                ...
                if (eos_token_id is not None) and (next_token.item() in eos_token_id):
                    ...
                    next_beam_tokens[batch_idx, beam_idx] = batch_beam_idx
                    beam_idx += 1
                ...
                self._done[batch_group_idx] == self._done[
                    batch_group_idx
                ] or self._beam_hyps[batch_beam_idx].is_done(
...
class BeamHypotheses:
    def __init__(
        ...
        if not isinstance(self.early_stopping, bool) and self.max_length is None:
            raise ValueError(
                "when `do_early_stepping` is set to a string, `max_length` must be defined. Ensure it is passed to the"
                " BeamScorer class instance at the initilization time."
            )
    ...
    def is_done(
        ...
        if self.early_stopping is True:
            ...
            if self.length_penalty > 0.0:
                ...
                highest_attainable_score = (
                    best_sum_logprobs
                    / (self.max_length - decoder_prompt_len)
                    * self.length_penalty
                )
            else:
                highest_attainable_score = (
...
class ConstrainedBeamSearchScorer(BeamScorer):
    ...
    def __init__(
        ...
        if not isinstance(num_beams, int) or num_beams <= 1:
            raise ValueError(
                f"`num_beams` has to be an interger striclty greater than 1, but is {num_beams}. For `num_beams` has to be"
                f" divisible by `num_beam_groups`, but is {num_beam_groups} with `num_beams` being {num_beams}"
            )
        if (
            not isinstance(num_beam_groups, int)
            ...
            raise ValueError(
                "`num_beam_groups` has to be an interger smaller or equal than `num_beams` and `num_beams` has to be"
                f"divisible by `num_beam_groups`, but is {num_beam_groups} with `num_beams` being {num_beams}"
            )
    ...
    def process(
        ...
        if not (batch_size == (input_ids.shape[0] // self.group_size)):
            if self.num_beam_groups > 1:
                raise ValueError(
                    f"A group beam size of {input_ids.shape[0]} is used as the input, but a group beam"
                    f" size of {self.group_size} is expected by the bea, scorer"
                )
            else:
                raise ValueError(
                    f"A beam size of {input_ids.shape[0]} is used as the input, but a beam size of "
                    f"{self.group_size} is expected b the beam scorer"
                )
        device = input_ids.device
        next_beam_scores = torch.zeros(
            (batch_size, self.group_size), dtype=next_scores.dtype, device=device
        ...
        for batch_idx, beam_hyp in enumerate(self._beam_hyps):
            ...
            for beam_token_rank, (next_token, next_score, next_index) in enumerate(zip(next_tokens[batch_idx], next_scores[batch_idx], next_indices[batch_idx])):
                ...
                if (eos_token_id is not None) and (next_token.item() in eos_token_id):
                    ...
                    if is_beam_token_worse_than_top_num_beams:
                        continue
                    complete_constraint = self.check_complete_constraints(input_ids[batch_beam_idx].cpu().tolist())
                    if complete_constraint:
                        if beam_indices is not None:
                            beam_index = beam_indices[batch_beam_idx]
                            beam_index = beam_index + (batch_beam_idx,)
                        else:
    ...
    def step_sentence_constraint(
            ...
            sent_beam_tokens: torch.FloatTensor,
        ...
        advance_contraint_states = self.make_constraint_states(orig_len)
        ...
        for seq_idx, pre_seq in enumerate(this_batch_input_ids):
            ...
            advance_state = advance_contraint_states[seq_idx]
            ...
            if not advance_state.completed:
                ...
                for advance_token in advance_tokens:
                    ...
                    if advance_seq in track_new["new_seqs"]:
