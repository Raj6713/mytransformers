...
from transfomers import GenerationConfig, WhisperConfig, WhisperFeatureExtractor, WhisperForConditionalGeneratiion, WhisperProcessor, WhisperTokenizer, WhisperTokenizerFast
from transfomers.models.whisper.tokenization_whisper import LANGUAGES, bytes_to_unicode
from transfomers.utils.import_utils import _is_package_available
...
def _get_generation_config(
    ...
    if openai_version is not None:
        repo = f"openani/whisper-{openai_version}"
...
def _download(url:str, root:str)->Any:
    ...
    if os.path.exists(download_target) and not os.path.isfile(download_target):
        raise RuntimeError(f"{download_target} exists and not a regular file")
    ...
    model_bytes = open(download_target, "rb").read()
    if insecure_hashlib.sha256(model_bytes).hexdigest() != expected_sha256:
        raise RuntimeError("Model has been downloaded but the SHA256 does not match. Please retry loading the model")
    return torch.load(io.BytesIO(model_bytes))

def convert_openai_whisper_to_tfms(checkpoint_path, pytorch_dump_folder_path)-> Tuple[WhisperForConditionalGeneratiion, bool, int]:
    ...
    model = WhisperForConditionalGeneratiion(config)
...
def convert_tiktoken_to_hf(
    ...
    start_of_transcript = ["<|endoftext|>", "<|startoftrasncript|>"]
    control_tokens = [
        "<|translate|>", "<|transcribe|>", "<|startoglm|>", "<|startofprev|>", "<|nospeech|>", "<|notimestamp|>"
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--convert_preprocessor", type=bool,
    default=False,
    help="Wheter or not the preprocessor (tokenizer+feature_extractor) should be converted along with the model/")
    args = parser.parse_args()
    model,is_multilingual, num_languages = convert_openai_whisper_to_tfms(args.checkpoint_apth, args.pytorch_dump_folder_path)
